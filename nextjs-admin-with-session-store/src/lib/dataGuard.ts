import { deepClone, hasCircularReference, isNullish, isObject, isString } from './helpers';

export type SensitiveContentKey = keyof typeof sensitiveContentRegExp;
type MaskingChar = 'X' | 'x' | '$' | '/' | '.' | '*' | '#' | '+' | '@' | '-' | '_' | ' ';

export interface IMaskDataOptions {
...
function maskSensitiveContent(
    ...
    const applyPatternMasking = (currentValue: string, pattern: RegExp): string => {
        ...
        currentValue.replace(skipMaskingPattern, (match, p1, offset) => {
            ...
            result += substring.replace(pattern, match => maskSensitiveValue(match, options as Partial<IMaskDataOptions>));
...
export function maskString(
    value: string,
    types: SensitiveContentKey[]|undefined, // This remains non-optional
    customSensitiveContentRegExp?: Record<string, RegExp>,
    options?: Partial<IMaskDataOptions>
): string;
...
function maskError(item: Error, options: Partial<IMaskDataOptions>): Error {
    ...
    maskedError.stack = maskString(item.stack as string, undefined, options.customPatterns, options);
    return maskedError;
}
...
function maskObject<T>(item: T, options: Partial<IMaskDataOptions>): T {
    ...
    return Object.entries(processedItem as Object).reduce(
        (acc, [key, value]) => {
...
function performMasking<T>(key: string, value: unknown, options: Partial<IMaskDataOptions>): T {
    ...
    if (isString(value)) {
        const { isUnmasked, content } = unmaskContent(value as string);
        if (isUnmasked) {
            return content as T; // return content without '##' and without masking
        }
    }
    return (
        options.keyCheck && options.keyCheck(key)
            ? isString(value)
                ? maskSensitiveValue(value as string, options) // Pass maskLength here
...
export function maskData<T>(item: T, options: Partial<IMaskDataOptions> = {}): T {
    ...
    if (isString(item)) {
        ...
        return maskString(item as string, undefined, options.customPatterns, options) as T;
    }
