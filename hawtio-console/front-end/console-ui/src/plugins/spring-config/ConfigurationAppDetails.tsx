...
function ConfigurationAppDetails() {
  ...
  const [paginatedRows, setPaginatedRows] = React.useState<ConfigurationProperty[]>(
    state.props.slice((page - 1) * perPage, page * perPage)
  );
  ...
  return (
    <>
      <PageSection variant={PageSectionVariants.light}>
        <TextContent>
          <Text component='h4'>
            Application Configuration: {state.application}/{state.profile}{' '}
          </Text>
          <Button
            onClick={() => {
              editConfig();
        ...
        <Card>
          ...
          <CardFooter>
            <div className='small form-group d-flex align-items-center justify-content-between mt-4 mb-0'>
              <Link to='/spring-config'>Go to configurations page</Link>
            </div>
          </CardFooter>
        </Card>
      </PageSection>
