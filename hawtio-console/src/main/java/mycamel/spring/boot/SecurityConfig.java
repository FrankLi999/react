...
import mycamel.spring.boot.core.security.csrf.SpaCsrfTokenRequestHandler;
import mycamel.spring.boot.core.security.filter.CheckOauth2TokenFilter;
import mycamel.spring.boot.core.security.filter.CookieCsrfFilter;
import mycamel.spring.boot.core.security.filter.SpaWebFilter;
import mycamel.spring.boot.core.security.jwt.JwtAuthorizationProperties;
import mycamel.spring.boot.core.security.support.GroupsClaimMapper;
import mycamel.spring.boot.core.security.support.NamedOidcUser;
...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChainResourceServer(HttpSecurity http) throws Exception {
		JwtIssuerAuthenticationManagerResolver authenticationManagerResolver = JwtIssuerAuthenticationManagerResolver
			.fromTrustedIssuers(String.format("https://login.microsoftonline.com/%s/v2.0", azureTenatId),
					String.format("https://%s.auth0.com/", auth0TenatId));
		...
		return http.securityMatcher("/spring/admin/resource/**")
			...
			// jwtCustomizer.jwkSetUri("https://login.microsoftonline.com/cddc1229-ac2a-4b97-b78a-0e5cacb5865c/v2.0")))
			.addFilterAfter(new CheckOauth2TokenFilter(), ChannelProcessingFilter.class)
			// One issuer
			// .oauth2ResourceServer(oauth2 -> oauth2.jwt(jwt ->
			// jwt.jwkSetUri("https://login.microsoftonline.com/cddc1229-ac2a-4b97-b78a-0e5cacb5865c/discovery/v2.0/keys?appid=cddc1229-ac2a-4b97-b78a-0e5cacb5865c")))
			// multiple issuer
	...
	public SecurityFilterChain filterChainOAuth2Client(HttpSecurity http) throws Exception {
		http.securityMatcher("/spring/admin/dashboard", "/spring/admin/dashboard/**", "/spring/admin/api/**",
				"/spring/admin/auth/**", "/login", "/error", "/actuator/**", "/oauth2/**", "/login/oauth2/**")
			.csrf(csrf -> csrf.ignoringRequestMatchers("/spring/admin/api/csrf")
				// XSRF-TOKEN cookie won’t be marked HTTP-only, so React can read it and
				// send it back when it tries to manipulate data
				...
				// The default behavior of Spring Security 6 is to postpone looking up the
				// CsrfToken until it is required.
			...
			authorizeHttpRequests((authz -> authz.requestMatchers("/actuator/**")
				...
				.requestMatchers("/spring/admin/dashboard", "/spring/admin/dashboard/index.html",
						"/spring/admin/dashboard/static/**", "/spring/admin/dashboard/*.ico",
						"/spring/admin/dashboard/*.json", "/spring/admin/dashboard/*.png")
				.permitAll()
				.requestMatchers("/spring/admin/api/**")
				.permitAll()
				.anyRequest()
				.authenticated()))
			// Configure a custom Filter to load the CsrfToken on every request, which
			// will return a new cookie if needed.
			.addFilterAfter(new CookieCsrfFilter(), BasicAuthenticationFilter.class)
			.addFilterAfter(new SpaWebFilter(), BasicAuthenticationFilter.class);
		http.formLogin(Customizer.withDefaults());
		http.oauth2Login(oauth2 -> oauth2.userInfoEndpoint(ep -> ep.oidcUserService(customOidcUserService(props))));
		return http.build();
	}
	...
	private OAuth2UserService<OidcUserRequest, OidcUser> customOidcUserService(JwtAuthorizationProperties props) {
		...
		final GroupsClaimMapper mapper = new GroupsClaimMapper(props.getAuthoritiesPrefix(), props.getGroupsClaim(),
