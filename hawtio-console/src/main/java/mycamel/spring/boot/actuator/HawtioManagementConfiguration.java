package mycamel.spring.boot.actuator;
...
public class HawtioManagementConfiguration {
	...
	// a path within Spring server or management server that's the "base" of hawtio
	// actuator.
	// By default it should be "/actuator/hawtio", but may be affected by
	// application.properties settings
	// (for example management.endpoints.web.base-path which defaults to "/actuator", but
	// can be customized)
	...
	public void initializeHawtioPlugins(final HawtioEndpoint hawtioEndpoint,
			final Optional<List<HawtioPlugin>> plugins) {
	...
	public SimpleUrlHandlerMapping hawtioWelcomeFiles(final EndpointPathResolver pathResolver) {
		...
		// order=10 to handle "/actuator/hawtio/" URL as forward to
		// "/actuator/hawtio/index.html" for consistency
	...
	// use @Order annotation to ensure filter mapping as in web.xml. method invocation
	// order _may_ be JVM dependent

	/**
	 * Since Spring Boot 3.0, paths with trailing slash are not automatically processed
	 * and need to be explicitly configured for handling them. This Spring Boot specific
	 * filter used to redirect {@code /actuator/hawtio/} requests to
	 * {@code /actuator/hawtio/index.html}, but now it used for consistency with WAR
	 * deployments - to redirect {@code /actuator/hawtio} to {@code /actuator/hawtio/}.
	 * Then the request is being processed by {@link ClientRouteRedirectFilter}.
	 */
	@Bean
	@Order(0)
	...
	// HawtioManagementConfiguration.JolokiaForwardingController which returns
	// "forward:/actuator/jolokia" view name handled by DispatcherServlet. Such request
	// (with original URI
	// /actuator/hawtio/jolokia/*) is already handled by all the above Hawtio filters, so
	// there's NO NEED
	// to map "/hawtio/jolokia/*" pattern to AuthenticationFilter, because filters will be
	// invoked by the forward
	//
	// when using /actuator/jolokia/* request, we invoke Jolokia Actuator endpoint
	// directly and NO Hawtio filters
	// will be invoked (which is fine), but we need AuthenticationFilter being mapped to
	// "/actuator/jolokia/*"

	/**
	 * {@link AuthenticationFilter} handling direct Jolokia Actuator endpoint requests
	 * ({@code /actuator/jolokia/*}).
	 * @param pathResolver
	 * @return
	 */
	@Bean
	...
	@ConditionalOnBean(JolokiaEndpoint.class)
	@ConditionalOnExposedEndpoint(name = "jolokia")
	public FilterRegistrationBean<AuthenticationFilter> jolokiaAuthenticationFilter(
			final EndpointPathResolver pathResolver) {
	...
	/**
	 * {@link AuthenticationFilter} handling proxy requests
	 * ({@code /actuator/hawtio/proxy/*}). No need to declare
	 * {@code /actuator/hawtio/jolokia/*} mapping here, as it'd be a duplication of
	 * {@link #jolokiaAuthenticationFilter} mapping.
	 * @return
	 */
	@Bean
	@Order(11)
	...
	/**
	 * This filter was called {@code LoginRedirectFilter}, but now it also handles
	 * redirection/forwarding for client-side routes (React Router), so we no longer need
	 * this special RegExp mapped
	 * {@link org.springframework.web.bind.annotation.RequestMapping} annotated method in
	 * {@link HawtioEndpoint}.
	 * @param redirector
	 * @param pathResolver
	 * @return
	...
	public FilterRegistrationBean<ClientRouteRedirectFilter> clientRouteRedirectFilter(final Redirector redirector,
		...
		String[] unpdatedUnsecuredPaths = new String[unsecuredPaths.length + 1];
        for (int i = 0; i < unsecuredPaths.length; i++) {
			unpdatedUnsecuredPaths[i] = unsecuredPaths[i];
		}
		unpdatedUnsecuredPaths[unsecuredPaths.length] = hawtioPath + "/assets"; //TODO
		final FilterRegistrationBean<ClientRouteRedirectFilter> filter = new FilterRegistrationBean<>();
		final ClientRouteRedirectFilter clientRouteRedirectFilter = new ClientRouteRedirectFilter(unpdatedUnsecuredPaths,
				pathResolver.resolve("hawtio"));
	...
	@Bean
	@Order(14)
	public FilterRegistrationBean<FlightRecordingDownloadFacade> flightRecorderDownloadFacade(
			final EndpointPathResolver pathResolver) {
	...
	public ServletRegistrationBean<ProxyServlet> jolokiaProxyServlet() {
		return new ServletRegistrationBean<>(new ProxyServlet(), hawtioPath + "/proxy/*");
	}

	@Bean
	public ServletRegistrationBean<KeycloakUserServlet> userServlet() {
		return new ServletRegistrationBean<>(new KeycloakUserServlet(), hawtioPath + "/user/*");
	}
	...
	public ServletRegistrationBean<LoginServlet> loginServlet(Redirector redirector) {
		...
		return new ServletRegistrationBean<>(loginServlet, hawtioPath + "/auth/login");
	}
	...
	public ServletRegistrationBean<LogoutServlet> logoutServlet(Redirector redirector) {
		...
		return new ServletRegistrationBean<>(logoutServlet, hawtioPath + "/auth/logout");
	}

	@Bean
	public ServletRegistrationBean<KeycloakServlet> keycloakServlet() {
		return new ServletRegistrationBean<>(new KeycloakServlet(), hawtioPath + "/keycloak/*");
	}

	@Bean
	public ServletRegistrationBean<AuthConfigurationServlet> oidcServlet() {
		return new ServletRegistrationBean<>(new AuthConfigurationServlet(), hawtioPath + "/auth/config/*");
	}
	...
	@Bean
	public ServletListenerRegistrationBean<SpringHawtioContextListener> hawtioContextListener(
			final ConfigManager configManager) {
		return new ServletListenerRegistrationBean<>(new SpringHawtioContextListener(configManager, hawtioPath));
	}
	...
	private String[] prependContextPath(String[] paths) {
		return Arrays.stream(paths).map(path -> hawtioPath + path).toArray(String[]::new);
	}
	...
	private static class JolokiaForwardingController extends AbstractUrlViewController {
		...
		protected String getViewNameForRequest(final HttpServletRequest request) {
			// Forward requests from hawtio/jolokia to the Spring Boot Jolokia actuator
			// endpoint
			final StringBuilder b = new StringBuilder();
			b.append("forward:");
			b.append(jolokiaPath);
			final String pathQuery = request.getRequestURI()
				.substring(request.getContextPath().length() + hawtioJolokiaPath.length());
			b.append(pathQuery);
