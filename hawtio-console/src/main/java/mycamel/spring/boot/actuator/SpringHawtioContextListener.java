...
import mycamel.spring.boot.actuator.security.SpringSecurityJAASConfiguration;
...
public class SpringHawtioContextListener extends HawtioContextListener {
	...
	@Override
	protected void configureAuthenticationProviders(ServletContext servletContext,
		...
		try {
			...
			while (context != null) {
				...
				Class<?> hawtioSpringSecurityLoginModule = Class.forName("security.HawtioSpringSecurityLoginModule");
				Map<String, ?> filterChains = context.getBeansOfType(sfcClass);
				if (!filterChains.isEmpty()) {
					// assume there's at least one SecurityFilterChain, usually configured
					// with
					// @EnableWebSecurity by @Bean method accepting
					// org.springframework.security.config.annotation.web.builders.HttpSecurity
					// builder.

					// we have to configure JAAS login module that'll integrate with
					// Spring Security
