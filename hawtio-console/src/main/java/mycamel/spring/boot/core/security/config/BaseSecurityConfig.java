package mycamel.spring.boot.core.security.config;
...
public abstract class BaseSecurityConfig {
	...
	protected BaseSecurityConfig sessionCreationPolicy(HttpSecurity http) throws Exception {
		// No session
		// http.sessionManagement(oauth2 ->
		// oauth2.sessionCreationPolicy(SessionCreationPolicy.STATELESS));
		return this;
	}

	protected BaseSecurityConfig logout(HttpSecurity http) throws Exception {
		// http.logout(authz -> authz.disable()); // we are stateless; so /logout endpoint
		// not needed

		// http.logout(logout -> logout.deleteCookies("JSESSIONID")
		// .logoutRequestMatcher(new
		// AntPathRequestMatcher("/spring/admin/dashboard/integrator/logout")))
	...
	protected BaseSecurityConfig exceptionHandling(HttpSecurity http) throws Exception {
		/***********************************************
		 * To prevent redirection to the login page when someone tries to access a
		 * restricted page
		 **********************************************/
		// http.exceptionHandling(authz -> authz.authenticationEntryPoint(new
