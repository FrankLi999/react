...
public class SpaWebFilter extends OncePerRequestFilter {
	...
	// private static final String API_NAMESPACE_REGEX =
	// "^/(spring/admin/api/.*|spring/admin/api)";
	...
	public void doFilterInternal(final HttpServletRequest req, final HttpServletResponse res, final FilterChain chain)
		...
		if (log.isDebugEnabled()) {
			log.debug("Logging Request  {} : {} : isApiNamespace: {}: isDashboard: {}: statusCode: {}", req.getMethod(),
					servletPath, isDashboard, isFilename, res.getStatus());
		}

		// if (!isApiNamespace && !isFilename && res.getStatus() == 404) {
		// Authentication user = SecurityContextHolder.getContext().getAuthentication();
		// if (user != null && !path.startsWith("/api") && !path.contains(".") &&
		// path.matches("/(.*)")) {
		// request.getRequestDispatcher("/").forward(request, response);
		// return;
		// }
		if (isDashboard && !isFilename) {
			req.getRequestDispatcher("/spring/admin/dashboard/index.html").forward(req, res);
		}
		else {
