...
public class RefreshConfigProcessor {
	...
	public void setupRequestContext(Exchange exchange) {
		CamelRequest<RefreshConfigRequest> camelRequest = this.camelUtils.processRequest(exchange,
				RefreshConfigRequest.class);
		exchange.getMessage()
			.setHeader(MyCamelConstants.HEADER_MY_CAMEL_AUTH_TOKEN, camelRequest.getRequestContext().getAuthToken());
	...
	public void createApiResponse(Exchange exchange) {
		Map<String, Object> requestContextMap = exchange.getProperty(MyCamelConstants.PROPERTY_MY_CAMEL_CONTEXT_MAP,
				Map.class);
		...
		if (log.isDebugEnabled()) {
			camelUtils.logApiResponsePayload(log, "MyCamel:API_RESPONSE:RefreshConfig (Exit)",
					requestContext.getRequestHeader().getTraceId(), responseJson);
		}
	...
	public void refreshConfig(Exchange exchange) throws JsonProcessingException {
		...
		resp.put(StringUtils.hasText(refreshConfig.getPod()) ? refreshConfig.getPod() : refreshConfig.getRefreshUrl(),
				propertiesUpdates);
