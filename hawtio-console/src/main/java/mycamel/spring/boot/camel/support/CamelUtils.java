...
public class CamelUtils {
	...
	public ResponseEntity<String> processCamelRequest(CamelRequest camelRequest, String camelRoute) {
		...
		headers.put(MyCamelConstants.HEADER_MY_CAMEL_TRACE_ID,
				camelRequest.getRequestContext().getRequestHeader().getTraceId());
		CamelResponse<String> camelResponse = producerTemplate.requestBodyAndHeaders(camelRoute, camelRequest, headers,
				CamelResponse.class);
		return ResponseEntity.status(camelResponse.getRequestContext().getStatus())
			.body(camelResponse.getResponseBody());
	}
	...
	public void logApiRequestPayload(Logger log, String logSubject, String traceId, Object payload) {
		if (log.isDebugEnabled()) {
			log.debug(String.format("LogSubject: %s, Payload: %s, TraceId: %s", logSubject,
					payload instanceof String ? payload : serializePayload(payload, log), traceId));
		}
	}
