...
export const RefreshConfig: React.FunctionComponent = () => {
  ...
  const refreshConfig = async () => {
    ...
    mpgPods.length > 0 &&
      mpgPods.split(',').forEach((pod) =>
        refreshConfigUrls.push({
          pod: pod,
          refreshUrl:
            pod.startsWith('http://') || pod.startsWith('https://')
              ? pod
              : `http://${pod}:8080/actuator/refresh`,
        })
      );
    ccpayPods.length > 0 &&
      ccpayPods.split(',').forEach((pod) =>
        refreshConfigUrls.push({
          pod: pod,
          refreshUrl:
            pod.startsWith('http://') || pod.startsWith('https://')
    ...
    <>
      <PageSection variant={PageSectionVariants.light}>
        <TextContent>
          ...
          <Text component='p'>
            Refresh Configurations of S2I MPG, CCPay and Services integrators
          </Text>
          (refreshR)
        </TextContent>
      </PageSection>
      <Form isHorizontal>
        <FormGroup
          label='s2i-mpg-integrator Pods'
          isRequired
          fieldId='horizontal-form-mpg-pods'
        >
          <TextInput
            value={mpgPods}
            isRequired
        ...
        <FormGroup
          label='s2i-ccpay-integrator Pods'
          isRequired
          fieldId='horizontal-form-ccpay-pods'
        >
          <TextInput
            value={ccpayPods}
            isRequired
        ...
        <FormGroup
          label='s2i-service-integrator Pods'
          isRequired
          fieldId='horizontal-form-service-pods'
        >
          <TextInput
            value={servicePods}
            isRequired
