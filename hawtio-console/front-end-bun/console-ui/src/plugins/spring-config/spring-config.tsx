...
export const SpringConfig: React.FunctionComponent = () => {
  ...
  const importConfigurations = (configIle: File) => {
    ...
    axios
      .post('/my-camel/admin/api/spring-config/import', formData, config)
      .then((response) => {
  ...
  const importSqlConfigurations = (configIle: File) => {
    ...
    axios
      .post('/my-camel/admin/api/spring-config/sql', formData, config)
      .then((response) => {
  ...
  const springConfigContent = () => {
    return (
      <>
        <PageGroup>
          <PageSection variant={PageSectionVariants.light}>
            ...
            <div className='small form-group d-flex align-items-center justify-content-start mt-4 mb-0'>
              ...
              <Button
                ...
                style={{ marginLeft: '12px' }}
                onClick={() => {
                  showImportSqlConfigurationModal();
                }}
              >
                Import SQL
              </Button>
          ...
          <PageSection>
            <Card>
              <Table
                variant='compact'
                aria-label='Spring configurations'
              >
                <Thead>
                  <Tr>
                    {configurationTableColumns.map((column, columnIndex) => (
                      <Th
                        key={columnIndex}
                        sort={column.sort ? getSortParams(columnIndex) : undefined}
                      >
                        {column.text}
                      </Th>
                    ))}
                ...
                <Tbody>
                  {paginatedRows.map((row: any, rowIndex: number) => (
                    <Tr key={rowIndex}>
                      {configurationTableColumns.map(
                        (
                          column // , columnIndex
                        ) => (
                          <Td key={column.dataField}>{renderColum(column, row)}</Td>
                        )
                      )}
                    </Tr>
                  ))}
        ...
        <DeleteConfirmation
          showModal={displayDeleteConfirmationModal}
          confirmModal={deleteAppConfiurationDetails}
          hideModal={hideDeleteConfirmationModal}
          row={deleteRow}
          message={`Are you sure to delete configurations for application for ${deleteRow?.application}/${deleteRow?.profile}`}
        />
        <ImportConfiguration
          showModal={displayImportConfirmationModal}
          importConfiguration={importConfigurations}
          hideModal={hideImportConfigurationModal}
        />
        <ImportConfiguration
          showModal={displayImportSqlConfirmationModal}
          importConfiguration={importSqlConfigurations}
          hideModal={hideImportSqlConfigurationModal}
        />
        <PageSection
          id='connect-main'
        ...
        <CookiesProvider>
          <MyConfigProvider>
            <ErrorBoundaryContextProvider>
              <ErrorBoundary>{springConfigContent()}</ErrorBoundary>
            </ErrorBoundaryContextProvider>
          </MyConfigProvider>
        </CookiesProvider>
      </>
