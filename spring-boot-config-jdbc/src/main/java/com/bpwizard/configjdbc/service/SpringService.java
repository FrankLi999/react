package com.bpwizard.configjdbc.service;
...
import com.bpwizard.configjdbc.dto.SpringMailData;
import com.bpwizard.configjdbc.entity.Role;
import com.bpwizard.configjdbc.entity.Tenant;
import com.bpwizard.configjdbc.entity.User;
import com.bpwizard.configjdbc.exception.SpringExceptionUtils;
import com.bpwizard.configjdbc.jdbc.JdbcUtils;
import com.bpwizard.configjdbc.security.*;
import com.bpwizard.configjdbc.security.annotation.UserEditPermission;
import com.bpwizard.configjdbc.security.jwt.JWTEncryptionService;
import com.bpwizard.configjdbc.security.jwt.JWTSignatureService;
import com.bpwizard.configjdbc.security.model.UserDto;
import com.bpwizard.configjdbc.validation.ChangePasswordForm;
import com.bpwizard.configjdbc.validation.ResetPasswordForm;
...
public abstract class SpringService
    ...
    public void createSpringService(SpringProperties properties,
                                    ...
                                    JWTSignatureService jwsTokenService,
                                    JWTEncryptionService jweTokenService,
...
//    }
    ...
    public void onStartup() {
        ...
        if (properties.getAccount() != null && properties.getAccount().isPopulate() && null != properties.getAccount().getUser() && properties.getAccount().getUser().length > 0) {
            try {

                // Check if the user already exists
                userDetailsService
                        .loadUserByUsername(properties.getAccount().getUser()[0].getUsername());
...
//				}
                ...
                SpringProperties.User[] users = properties.getAccount().getUser();
...
//				"user", WebUtils.currentUser());
    ...
    public void verifyUser(ID userId, String verificationCode) {
        ...
        JWTClaimsSet claims = jweTokenService.parseToken(verificationCode,
                JWTEncryptionService.VERIFY_AUDIENCE, user.getCredentialsUpdatedMillis());
    ...
    protected void mailChangeEmailLink(U user) {
        ...
        String changeEmailCode = jweTokenService.createToken(
                JWTEncryptionService.CHANGE_EMAIL_AUDIENCE,
...
//	}
    ...
    public String fetchNewToken(Optional<Long> expirationMillis,
        ...
        return SecurityUtils.TOKEN_PREFIX +
                jwsTokenService.createToken(JWTSignatureService.AUTH_AUDIENCE, username,
                        expirationMillis.orElse(properties.getJwt().getExpirationMillis()));
    }
    ...
    public Map<String, String> fetchFullToken(String authHeader) {
        ...
        SecurityUtils.ensureCredentials(jwsTokenService.parseClaim(authHeader.substring(SecurityUtils.TOKEN_PREFIX_LENGTH),
                JWTSignatureService.USER_CLAIM) == null,	"com.bpwizard.spring.fullTokenNotAllowed");

        UserDto currentUser = WebUtils.currentUser();

        Map<String, Object> claimMap = Collections.singletonMap(JWTSignatureService.USER_CLAIM,
        ...
        Map<String, String> tokenMap = Collections.singletonMap("token", SecurityUtils.TOKEN_PREFIX +
                jwsTokenService.createToken(JWTSignatureService.AUTH_AUDIENCE, currentUser.getUsername(),
    ...
    public void addAuthHeader(HttpServletResponse response, String username, Long expirationMillis) {
        ...
        response.addHeader(SecurityUtils.TOKEN_RESPONSE_HEADER_NAME, SecurityUtils.TOKEN_PREFIX +
                jwsTokenService.createToken(JWTSignatureService.AUTH_AUDIENCE, username, expirationMillis));
    }
